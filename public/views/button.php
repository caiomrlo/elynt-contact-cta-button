...
$type = isset($button['type']) ? $button['type'] : 'inline';
...
if (!empty($initial_message)) {
	$whatsapp_url .= "?text=" . rawurlencode($initial_message);
}

// Construct classes
$classes = array('ecb-whatsapp-button');
if ($type === 'fixed') {
	$classes[] = 'ecb-fixed';
	$classes[] = 'ecb-pos-' . $position;
} else {
	$classes[] = 'ecb-inline';
}

if ($layout === 'icon_only') {
	$classes[] = 'ecb-icon-only';
}
...
<a href="<?php echo esc_url($whatsapp_url); ?>" class="<?php echo esc_attr($class_string); ?>"
	style="<?php echo esc_attr($style); ?>" target="_blank" rel="noopener noreferrer">
	<svg style=" fill: <?php echo esc_attr($text_color); ?>;" xmlns="http://www.w3.org/2000/svg"
	viewBox="0 0 640
